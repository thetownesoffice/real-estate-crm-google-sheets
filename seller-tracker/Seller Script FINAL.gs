// ============================================================
//  SELLER TRACKER — Final Script v2
//  📅 Date Picker | 🔍 Search | 🔃 Sort + Auto Hyperlinks
//
//  Column Map:
//  A=Name, B=Status, C=Priority, D=Last Contact, E=Follow-Up Count
//  F=Phone, G=Email, H=Address, I=Property Type, J=Bed/Bath
//  K=Property Value, L=Listing Price, M=Days on Market, N=Listing Date
//  O=Excl Right to Sell, P=Offer Received, Q=Offer Amt, R=Commission
//  S=Closing Date, T=Referral, U=Agent Notes
//
...
function onOpen() {
  ...
  ui.createMenu('🔍 Search')
    .addItem('Search Seller', 'runSearch')
...
//  F=6 Phone, G=7 Email, H=8 Address
// ════════════════════════════════════════════════════════════

function onEdit(e) {
  var sheet = e.range.getSheet();
  if (sheet.getName() !== "Seller List") return;
...
function sortByName() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Seller List");
  var lastRow = sheet.getLastRow();
  if (lastRow < 3) return;
  sheet.getRange(3, 1, lastRow - 2, 21).sort({ column: 1, ascending: true });
  SpreadsheetApp.getUi().alert("✅ Sorted A → Z by seller name.");
}
...
function sortByDate() {
  ...
  var lastRow = sheet.getLastRow();
  if (lastRow < 3) return;
  sheet.getRange(3, 1, lastRow - 2, 21).sort({ column: 4, ascending: true });
  SpreadsheetApp.getUi().alert("✅ Sorted by Last Contact Date — oldest at top.");
}

// ════════════════════════════════════════════════════════════
//  SEARCH — reads directly from cell C21 on Seller Dashboard
// ════════════════════════════════════════════════════════════

function runSearch() {
  var dash  = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("📊 Seller Dashboard");
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Seller List");

  var searchTerm = dash.getRange("C21").getValue().toString().trim();

  if (!searchTerm) {
    SpreadsheetApp.getUi().alert("Please type a seller name in the yellow search box on the Dashboard tab first.");
    return;
  }
  ...
  if (matches.length === 0) {
    SpreadsheetApp.getUi().alert('No seller found matching "' + searchTerm + '". Check spelling and try again.');
    return;
  }

  var thick = SpreadsheetApp.BorderStyle.SOLID_MEDIUM;
  for (var m = 0; m < matches.length; m++) {
    var range = sheet.getRange(matches[m], 1, 1, 21);
  ...
  var msg = matches.length === 1
    ? "✅ Found! " + sheet.getRange(matches[0], 1).getValue() + " is highlighted in bold. Use 'Clear Highlights' when done."
...
function goToNextMatch() {
  ...
  if (!matchRowsJson) {
    SpreadsheetApp.getUi().alert("No active search. Please run Search Seller first.");
    return;
  }
...
function clearSearchFormatting(sheet, totalRows) {
  var range = sheet.getRange(3, 1, totalRows, 21);
  range.setFontWeight("normal");
  var thin = SpreadsheetApp.BorderStyle.SOLID;
  range.setBorder(true, true, true, true, true, true, "#CCCCCC", thin);
}

// ════════════════════════════════════════════════════════════
//  CALENDAR DATE PICKER
//  D=4 Last Contact, N=14 Listing Date, S=19 Closing Date
// ════════════════════════════════════════════════════════════
...
function openCalendar() {
  ...
  if (col !== 4 && col !== 14 && col !== 19) {
    SpreadsheetApp.getUi().alert("Please click on a date column first:\n• Last Contact Date (column D)\n• Listing Date (column N)\n• Closing Date (column S)");
    return;
  }
...
function clearDateInCell(cellAddress) {
  SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Seller List").getRange(cellAddress).clearContent();
}
